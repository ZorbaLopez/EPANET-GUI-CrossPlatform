...
{$ENDIF}
  ...
  Forms, StdCtrls, Printers, ExtCtrls, Buttons, Dialogs, System.UITypes,
  XPForm, ResourceStrings;
...
type
  ...
  public
    ...
    function GetPage(const Index: Integer): {$IFDEF WINDOWS} TPrintPage {$ELSE} TImage {$ENDIF};
  end;
  ...
  private
    ...
    {$ELSE}
    fPage          : TImage;        {Current page's image}
    {$ENDIF}
    ...
    property fCanvas : TCanvas read GetCanvas; {Canvas of current page}
    {$ENDIF}
...
Var
  I: Integer;
  Page: {$IFDEF WINDOWS} TPrintPage {$ELSE} TImage {$ENDIF};
...
function TPageList.GetPage(const Index: Integer): {$IFDEF WINDOWS} TPrintPage {$ELSE} TImage {$ENDIF};

    { Retrieve pointer to page Index }

Begin
  Result := {$IFDEF WINDOWS} TPrintPage {$ELSE} TImage {$ENDIF}(Items[Index-1]);
End;
...
With fCanvas Do
  Begin
  Font.Assign(Value);
  {$IFDEF WINDOWS}Windows{$ELSE}LCLIntf{$ENDIF}.GetTextMetrics(Handle,TextMetrics);
  TableHorizOffset :=
...
Begin
  ...
  begin
    ...
    {$ELSE}
    fPage := TImage.Create(self);
    fPage.Width := PageWidthPixels;
    fPage.Height := PageHeightPixels;
    fCanvas.Clear;
    fCanvas.Brush.Color:=clNone;
    fCanvas.FillRect(fPage.BoundsRect);
    {$ENDIF}
...
procedure TPrintControl.FinishPage;
  ...
  Begin
  if (not fDoneDrawing) and ({$IFDEF WINDOWS} fCanvas {$ELSE} fPage {$ENDIF} <> nil) then
    Begin
...
procedure TPrintControl.PrintAll;
   ...
   if (PageCount > 0) and (Printer.Printers.Count > 0) then
      ...
      if fShowProgress and not Assigned(fProgressForm) then
         ...
         try
            ...
            begin
               ...
               begin
                  ...
                  Label1.Caption := Format(TXT_PR_PAGE,[i, PageCount]);
                  Update;
               end;

               { Draw the page on the printer's canvas. }
               if i > 1 then Printer.NewPage;
               Printer.Canvas.Draw(0, 0, fPages.GetPage(i){$IFNDEF WINDOWS}.Picture.Graphic{$ENDIF});
               Inc(i);
            end;
         ...
         begin
            MessageDlg(TXT_PR_ABORT, mtInformation, [mbOK], 0);
            fCancelPrinting := False;
         end;
...
if (Page > 0) AND (Page <= PageCount) then
  ...
  if fPreviewForm.ShwGrdBtn.Down then
    ...
    fPreviewForm.PaintBox1.Canvas.StretchDraw(r, fPages.GetPage(Page){$IFNDEF WINDOWS}.Picture.Graphic{$ENDIF});
    fPreviewForm.Panel2.Caption := Format(TXT_PG_NO,
                                          [Page, PageCount]);
...
begin
  DoneDrawing;
  try
    Printer.Canvas.Draw(0, 0, fPages.GetPage(1){$IFNDEF WINDOWS}.Picture.Graphic{$ENDIF});
  finally
  end;
  BeginJob;
end;
...
Begin
   ...
   PixelsPerInchVertical := Printer.YDPI;
   PixelsPerInchHorizontal := Printer.XDPI;

   { Get the physical size of the page. }
   TotalPageWidthPixels := Printer.PaperSize.PaperRect.PhysicalRect.Right;
   TotalPageHeightPixels := Printer.PaperSize.PaperRect.PhysicalRect.Bottom;

   { Get the printable size of the page. }
   PageWidthPixels := Printer.PaperSize.PaperRect.WorkRect.Width;
   PageHeightPixels := Printer.PaperSize.PaperRect.WorkRect.Height;

   { Get the gutters on the left, top, right, and bottom. }
   GutterLeft := Printer.PaperSize.PaperRect.WorkRect.Left;
   GutterTop  := Printer.PaperSize.PaperRect.WorkRect.Top;
...
{$IFNDEF WINDOWS}
function TPrintControl.GetCanvas: TCanvas;
begin
  Result := fpage.Canvas;
end;
{$ENDIF}
