...
{$ENDIF}
  ...
  StdCtrls, ComCtrls, Checklst, Clipbrd, TAGraph, TASeries, TACustomSeries,
  System.UITypes, Xprinter, Uglobals, Uutils, TATypes, TACustomSource,
  TAChartUtils, ResourceStrings;
...
//Clear report components
  Memo1.Clear;
  FreeChart1Series;
  TChartSeries(Chart2.Series[0]).Clear;
  TChartSeries(Chart2.Series[1]).Clear;
...
//Save information about variable being calibrated
  ...
  begin
    ...
    if RptVar = NODEQUAL then
      vname := MainForm.BrowserForm.NodeViewBox.Items[NODEQUAL]
    else
...
begin
  //Lazarus - Set titles
  Series3.Title := TXT_COMPUTED;
  Series4.Title := TXT_OBSERVED;
...
//If there are any data, then
  ...
  //Add obs. & simul. means to Mean Comparison chart
    {Chart2.Series.Add(stats[3],ID,clDefault);
    Chart2.Series.Add(stats[2],ID,clDefault);}
    Series3.Add(stats[3],ID,clDefault);
    Chart2.AddSeries(Series3);
    Series4.Add(stats[2],ID,clDefault);
...
//and update error statistics
  ...
  begin
    {with Chart1 do
      Series[SeriesCount-1].AddXY(v, v1, '', clDefault);}
    with Chart1 do
      TChartSeries(Series[SeriesCount-1]).AddXY(v, v1, '', clDefault);
    {aSeries := TChartSeries.Create(self);
    aSeries.AddXY(v, v1, '', clDefault);
    Chart1.AddSeries(aSeries);}
...
// Copy to file if file name supplied, otherwise copy to clipboard
  if Length(Fname) > 0 then
    theChart.SaveToWMF(Fname)
  else
    theChart.CopyToClipBoardMetaFile;
{$ELSE}
var
  theChart: TChart;
  vMs:TMemoryStream;
...
// Create a stringlist to hold the data
      ...
      begin
        ...
        begin
          for J := 0 to Count -1 do
            ...
            Slist.Add(Title + #9 + FloatToStr(GetXValue(J)) + #9 +
               FloatToStr(GetYValue(J)));
        end;
      end;
    end
  ...
  // Add data from Chart2 (mean comparisons) to list
    ...
    begin
      ...
      for J := 0 to TChartSeries(Series[0]).Count-1 do
        Slist.Add(PChartDataItem(TChartSeries(Series[0]).ListSource.Item[J])^.text + #9 +
                  FloatToStr(TChartSeries(Series[0]).GetYValue(J)) + #9 +
                  FloatToStr(TChartSeries(Series[1]).GetYValue(J)));
    end;
