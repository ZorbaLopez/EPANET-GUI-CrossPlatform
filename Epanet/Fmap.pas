...
uses
{$IFDEF WINDOWS}
  Windows, Messages, TADrawerWMF, LConvEncoding,
{$ELSE}
  LCLIntf, LCLType, LMessages, fpvectorial, fpvtocanvas, BGRABitmap,
  BGRABitmapPack, BGRABitmapTypes,
{$ENDIF}
  SysUtils, Classes, Graphics, Controls,
  Forms, Dialogs, Spin, StdCtrls, ExtCtrls, Buttons, Menus,
  Clipbrd, Math, Printers, Types, System.UITypes,
  Xprinter, Uglobals, Uutils, Umap, ResourceStrings;
...
////    procedure CreateParams(var Params: TCreateParams); override;
    ...
    procedure LegendPanelPaint(Sender: TObject);
    ...
    procedure DragLegend(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure MoveLegend(Sender: TObject; Shift: TShiftState; X, Y: Integer);
    procedure LegendMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
  ...
  private
    ...
    //Lazarus - used for dragging legends
    oldPos : TPoint;
    DraggedNode : Boolean;
    DraggedLink : Boolean;
    DraggedTime : Boolean;
  ...
  public
    ...
    {$ELSE}
    HiliteBitmap   : TBGRABitmap;            //TBGRABitmap storing highlighted rectangle
    {$ENDIF}
...
procedure TMapForm1.FormCreate(Sender: TObject);
//-------------------------------------------------
...
//Initialize object selector & highlighting variables
  ...
  {$ELSE}
  HiliteBitmap := TBGRABitmap.Create;
  {$ENDIF}
...
procedure TMapForm1.FormDestroy(Sender: TObject);
//----------------------------------------------
...
procedure TMapForm1.FormActivate(Sender: TObject);
//-----------------------------------------------
...
begin
  ...
  MainForm.MapHeader.Color := clActiveCaption;
  MainForm.MapCaption.Color := clActiveCaption;
  MainForm.MapCaption.Font.Color := clCaptionText;
  MainForm.FormActive := TXT_MAP;
  MainForm.FormActivated(self);
end;


procedure TMapForm1.FormResize(Sender: TObject);
//--------------------------------------------
...
begin
  ...
  try
    if NumFencePts > 0 then EraseFenceline;
...
{$IFDEF WINDOWS}
procedure TMapForm1.WMGetMinMaxInfo(var Msg: TWMGetMinMaxInfo);
//----------------------------------------------------------
...
procedure TMapForm1.SetPanelPos;
// ---------------------------------------------------------------------
// Re-locates legends and Time Panel on MapForm1 after its been re-sized.
//----------------------------------------------------------------------
begin
  if LinkLegendFrame.Y = MISSING then
    LinkLegendFrame.Y := (NodeLegendPanel.Height+1)/MainForm.MapForm.ClientHeight;
  with NodeLegendPanel do
    PlacePanel(NodeLegendPanel, Round(NodeLegendFrame.X*MainForm.MapForm.ClientWidth),
               Round(NodeLegendFrame.Y*MainForm.MapForm.ClientHeight), Width, Height);
  with LinkLegendPanel do
    PlacePanel(LinkLegendPanel, Round(LinkLegendFrame.X*MainForm.MapForm.ClientWidth),
               Round(LinkLegendFrame.Y*MainForm.MapForm.ClientHeight), Width, Height);
  with TimeLegendPanel do
    PlacePanel(TimeLegendPanel, Round(TimeLegendFrame.X*MainForm.MapForm.ClientWidth-Width),
               Round(TimeLegendFrame.Y*MainForm.MapForm.ClientHeight), Width, Height);
end;


procedure TMapForm1.PlacePanel(aPanel: TPanel; X, Y, W, H: Integer);
//-------------------------------------------------------------------
// Positions aPanel so that it falls within bounds of MapForm1 window.
//-------------------------------------------------------------------
begin
  if X < 0 then X := 0;
...
procedure TMapForm1.FormPaint(Sender: TObject);
//----------------------------------------------------
...
procedure TMapForm1.InvalidateMap(var aRect: TRect);
//-----------------------------------------------------
...
begin
  ...
  OffsetRect(aRect, -30, -30);
  InflateRect(aRect,60,60);        //Inflate aRect a little
...
procedure TMapForm1.RedrawMap;
//-------------------------------
// Redraws map at current extent.
//-------------------------------
begin
  if  (Assigned(MainForm.BrowserForm))            //Display hourglass if
  and (not MainForm.BrowserForm.VCRTimer.Enabled) //  not animating map
...
procedure TMapForm1.DrawObject(const ObjType: Integer; const Index: Integer);
//---------------------------------------------------------------
...
procedure TMapForm1.DrawDotLink(const P1: TPoint; const P2: TPoint);
//--------------------------------------------
...
begin
  with Canvas do
  begin
    Pen.Mode := {$IFDEF LINUX} pmNotXor {$ELSE} pmXor {$ENDIF};   {pmNotXor;}
    Pen.Style := psDot;
    MoveTo(P1.X,P1.Y);
    LineTo(P2.X,P2.Y);
    Pen.Style := psSolid;
...
procedure TMapForm1.DrawDotRect(aRect: TRect);
//--------------------------------------------
...
procedure TMapForm1.DrawFenceline;
//---------------------------------------------------
...
procedure TMapForm1.EraseFenceline;
//---------------------------------
// Erases a fenceline.
//----------------------------------
begin
...
procedure TMapForm1.ModifyMapDimensions;
//--------------------------------------------------------
...
procedure TMapForm1.OpenMapBackdrop;
//---------------------------------
...
procedure TMapForm1.ToolButtonClick(ButtonTag: Integer);
//----------------------------------------------------
...
procedure TMapForm1.FormMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
//-----------------------------------------------------
...
// or activates one of the popup menus
  ...
  // If object was selected, then display object's popup menu
    ...
    begin
      MainForm.BrowserForm.UpdateBrowser(SelectedObject,SelectedIndex);
...
procedure TMapForm1.FormMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
//-------------------------------------------------------
...
procedure TMapForm1.FormMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
//-------------------------------------------------------
...
procedure TMapForm1.FormMouseWheel(Sender: TObject; Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
//-----------------------------------------------------
...
procedure TMapForm1.FormDblClick(Sender: TObject);
//-----------------------------------------------------
...
begin
  ...
  then MainForm.BrowserForm.BtnEditClick(Sender);
end;


procedure TMapForm1.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
//-----------------------------------------------------
...
begin
  ...
  begin
    ...
    or (NumFencePts > 0))
    then MainForm.BrowserForm.BtnDeleteClick(Sender)
  end

  else if (Key = VK_F1) then MainForm.LaunchHelp(108)

  else with MainForm.PropEditForm do
    if Visible then
    begin
      SendMessage(Editor.Handle,{$IFDEF WINDOWS}WM_KEYDOWN{$ELSE}LM_KEYDOWN{$ENDIF},Key,0);
      Editor.Edit;
    end;
end;


procedure TMapForm1.Timer2Timer(Sender: TObject);
//----------------------------------------------
...
procedure TMapForm1.GoFlyover(const X,Y: Integer);
//----------------------------------------------------
...
procedure TMapForm1.DisplayFlyOver(const X,Y: Integer);
//---------------------------------------------------
...
procedure TMapForm1.GoLabeling(X: Integer; Y: Integer);
//--------------------------------------------------------------
...
// Create a borderless LabelForm that contains an Edit control
  ...
  // Position the Edit control at the current mouse location
    Top := MainForm.MapForm.ClientToScreen(p).Y;
    Left := MainForm.MapForm.ClientToScreen(p).X;
    BringToFront;
...
procedure TMapForm1.GoLinking(X: Integer; Y: Integer);
//--------------------------------------------------
...
function TMapForm1.BeginLinking(const X: Integer; const Y: Integer): Boolean;
//------------------------------------------------------
...
procedure TMapForm1.EndLinking(const X: Integer; const Y: Integer);
//------------------------------------------------
...
procedure TMapForm1.BeginFencing(X: Integer; Y: Integer);
//----------------------------------------------------
...
procedure TMapForm1.EndFencing;
//-------------------------------------------------
...
procedure TMapForm1.BeginZooming(X: Integer; Y: Integer);
//-----------------------------------------------------
...
// Prepare form's canvas for drawing the zoom rectangle
  Canvas.Pen.Mode := {$IFDEF LINUX} pmNotXor {$ELSE} pmXor {$ENDIF};
...
procedure TMapForm1.DrawZoomRect;
//--------------------------------------
...
procedure TMapForm1.GoZoomIn(X: Integer; Y: Integer);
//--------------------------------------
...
procedure TMapForm1.GoZoomOut(X: Integer; Y: Integer);
//----------------------------------
...
procedure TMapForm1.UpdateZoomFactor;
//-------------------------------------------------
...
procedure TMapForm1.BeginPanning(X: Integer; Y: Integer);
//-----------------------------------------------------------
// Processes the MouseDown action that begins panning of map.
//-----------------------------------------------------------
begin
// Change cursor shape (by forcing ButtonUp message)
  SendMessage(Handle,{$IFDEF WINDOWS}WM_LBUTTONUP{$ELSE}LM_LBUTTONUP{$ENDIF},0,0);
  Cursor := crFIST;
  HiliteOff;
...
procedure TMapForm1.GoPanning(X: Integer; Y: Integer);
//----------------------------------------------
...
procedure TMapForm1.EndPanning;
//--------------------------------------------------------
...
procedure TMapForm1.BeginAligning(Sender: TObject);
//-------------------------------------------------------
...
procedure TMapForm1.BeginSelecting(X: Integer; Y: Integer);
//-------------------------------------------------------
...
procedure TMapForm1.EndSelecting;
//-----------------------------------------------------
...
// Finish up moving a node or label
  ...
  begin
    ...
    MainForm.BrowserForm.UpdateBrowser(HiliteObject, HiliteIndex);
  end;
  Moving := False;
...
procedure TMapForm1.BeginVertexing;
//----------------------------------------------
...
procedure TMapForm1.GoVertexing(X: Integer; Y: Integer);
//-----------------------------------------------------
...
procedure TMapForm1.EndVertexing;
//-----------------------------------------------
...
procedure TMapForm1.DeleteVertex;
//----------------------------------------------------------
...
function TMapForm1.FindVertex(P: TPoint): Boolean;
//------------------------------------------------------
...
function TMapForm1.GetVertexRect(aVertex: PVertex): TRect;
//-----------------------------------------------------------
...
function TMapForm1.DrawVertices(DrawFlag: Boolean): TRect;
//---------------------------------------------------------
...
procedure TMapForm1.MoveVertex(const X,Y: Extended);
//----------------------------------------------
...
function TMapForm1.FindObject(const X: Integer; const Y: Integer): Boolean;
//---------------------------------------------------------
...
function TMapForm1.FindNode(const X: Integer; const Y: Integer): Boolean;
//-------------------------------------------------------
...
function TMapForm1.FindLabel(const X: Integer; const Y: Integer): Boolean;
//---------------------------------------------------------
...
function TMapForm1.FindLink(const X: Integer; const Y: Integer): Boolean;
//-------------------------------------------------------
...
procedure TMapForm1.EraseObject(const ObjType: Integer; const Index: Integer);
//---------------------------------------------------
...
procedure TMapForm1.MoveNode(const ObjType, ObjIndex: Integer;
  const X,Y: Extended);
//------------------------------------------------------------------
...
procedure TMapForm1.MoveLabelRect(const X,Y: Integer);
//----------------------------------------------------
...
procedure TMapForm1.MoveLabel(const Index: Integer);
//--------------------------------------------------
...
procedure TMapForm1.EraseLabel(const Index: Integer);
//----------------------------------------
...
procedure TMapForm1.ReplaceLabel(const Index: Integer; const S: String);
//----------------------------------------
...
procedure TMapForm1.ChangeHiliteObject(const ObjType, Index: Integer);
//-------------------------------------------------------------------
...
// the highlighting timer if Blinking option is on.
  ...
  begin
    ...
    begin
      ...
      {$ELSE}
      SetSize(HiliteRect.Right - HiliteRect.Left, HiliteRect.Bottom - HiliteRect.Top);
      {$ENDIF}
...
procedure TMapForm1.Timer1Timer(Sender: TObject);
//-------------------------------------------------
...
procedure TMapForm1.PopupMenu1Popup(Sender: TObject);
//------------------------------------------------------------
...
procedure TMapForm1.PopupCopyClick(Sender: TObject);
//-----------------------------------------------------------
...
procedure TMapForm1.PopupPasteClick(Sender: TObject);
//-----------------------------------------------------------
...
procedure TMapForm1.PopupDeleteClick(Sender: TObject);
//---------------------------------------------------
// OnClick handler for PopupDelete menu option.
// Deletes currently selected map item.
//---------------------------------------------------
begin
  MainForm.BrowserForm.BtnDeleteClick(Sender);
end;


procedure TMapForm1.PopupReverseClick(Sender: TObject);
//----------------------------------------------
...
procedure TMapForm1.PopupVerticesClick(Sender: TObject);
//-----------------------------------------------------
...
procedure TMapForm1.PopupMenu2Popup(Sender: TObject);
begin
...
procedure TMapForm1.PopupOptionsClick(Sender: TObject);
begin
  SetMapOptions;
  HiliteOn;
end;
...
procedure TMapForm1.PopupNodeLegendClick(Sender: TObject);
begin
  ToggleNodeLegend;
  HiliteOn;
end;
...
procedure TMapForm1.PopupLinkLegendClick(Sender: TObject);
begin
  ToggleLinkLegend;
  HiliteOn;
end;
...
procedure TMapForm1.PopupTimeLegendClick(Sender: TObject);
begin
  ToggleTimeLegend;
  HiliteOn;
end;
...
procedure TMapForm1.PopupBackdropClick(Sender: TObject);
begin
...
procedure TMapForm1.SetMapOptions;
//----------------------------------------------
...
begin
  ...
  try
    OldColor := MainForm.MapForm.Color;
    ...
    begin
      ...
      MainForm.MapForm.Color := MapBackColor[Map.Options.ColorIndex];
      MainForm.MapForm.Canvas.Pen.Color := MapForeColor[Map.Options.ColorIndex];
      NodeHiliteSize := MaxIntValue([PIXTOL,Map.Options.NodeSize+1]);
      LinkHiliteSize := MaxIntValue([PIXTOL,Map.Options.LinkSize+1]);
      if OldColor <> MainForm.MapForm.Color then Map.RedrawBackdrop;
      RedrawMap;
      DrawNodeLegend;
      DrawLinkLegend;
    end;
...
procedure TMapForm1.PopupMenu3Popup(Sender: TObject);
begin
  PopupDeleteVertex.Enabled := (SelectedVertex <> nil);
end;


procedure TMapForm1.PopupAddVertexClick(Sender: TObject);
begin
  AddVertex;
end;


procedure TMapForm1.PopupDeleteVertexClick(Sender: TObject);
begin
  DeleteVertex;
end;


procedure TMapForm1.PopupQuitEditingClick(Sender: TObject);
begin
  MainForm.SelectorButtonClick;
end;
...
procedure TMapForm1.LinkLegendBoxDblClick(Sender: TObject);
//--------------------------------------------------------
...
procedure TMapForm1.NodeLegendBoxDblClick(Sender: TObject);
//--------------------------------------------------------
...
procedure TMapForm1.ToggleLinkLegend;
//---------------------------------------
...
procedure TMapForm1.ToggleNodeLegend;
//---------------------------------------
...
procedure TMapForm1.ModifyNodeLegend;
//--------------------------------------------------------
...
procedure TMapForm1.ModifyLinkLegend;
//--------------------------------------------------------
...
procedure TMapForm1.DragLegend(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
//-------------------------------------------------------
// OnMouseDown event handler for the NodeLegendBox,
// the LinkLegendBox and TimeLegendPanel.  Left button
// starts dragging, right button invokes Legend Editor.
//-------------------------------------------------------
begin
  FormActivate(self);
...
procedure TMapForm1.MoveLegend(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
//-------------------------------------------------------
// OnMouseMove event handler for the NodeLegendBox,
// the LinkLegendBox and TimeLegendPanel. Left button
...
begin
  if (Sender = NodeLegendBox) and DraggedNode then
    ...
    begin
      NodeLegendBox.Hide;
      Left := Left - OldPos.X + X;
      Top := Top - OldPos.Y + Y;
      if (Left + Width) > self.Width then Left := self.Width - Width
      else if Left < 0 then Left := 0;
...
procedure TMapForm1.LegendMouseUp(Sender: TObject; Button: TMouseButton;
                                         Shift: TShiftState; X, Y: Integer);
//-------------------------------------------------------
// OnMouseUp event handler for the NodeLegendBox,
// the LinkLegendBox and TimeLegendPanel. It finishes
// dragging, and saves position in Uglobals.
//-------------------------------------------------------
begin
  if Sender = NodeLegendBox then
    with NodeLegendPanel do
    begin
      Canvas.Clear;
      NodeLegendBox.Show;
      NodeLegendFrame.X := Left / MainForm.MapForm.ClientWidth;
      NodeLegendFrame.Y := Top / MainForm.MapForm.ClientHeight;
      DraggedNode:=False;
      NodeLegendPanel.Repaint;
    end
  else if Sender = LinkLegendBox then
    ...
    begin
      Canvas.Clear;
      LinkLegendBox.Show;
      LinkLegendFrame.X := Left / MainForm.MapForm.ClientWidth;
      LinkLegendFrame.Y := Top / MainForm.MapForm.ClientHeight;
      DraggedLink:=False;
      LinkLegendPanel.Repaint;
    end
  else if Sender = TimeLegendPanel then
    with TimeLegendPanel do
    begin
      TimeLegendFrame.X := (Left + Width) / MainForm.MapForm.ClientWidth;
      TimeLegendFrame.Y := Top / MainForm.MapForm.ClientHeight;
      DraggedTime:=False;
      TimeLegendPanel.Repaint;
    end;
end;
...
procedure TMapForm1.LegendPanelPaint(Sender: TObject);
//-----------------------------------
// OnPaint handler for LegendPanels.
// Redraws the Node Legend.
//-----------------------------------
begin
  if DraggedNode then
  begin
    NodeLegendPanel.Canvas.Rectangle(0,0,NodeLegendPanel.width,NodeLegendPanel.height);
  end;
  if DraggedLink then
  begin
    LinkLegendPanel.Canvas.Rectangle(0,0,LinkLegendPanel.width,LinkLegendPanel.height);
  end;
end;


procedure TMapForm1.NodeLegendBoxPaint(Sender: TObject);
//-----------------------------------
// OnPaint handler for NodeLegendBox.
// Redraws the Node Legend.
//-----------------------------------
begin
  if DraggedNode then exit;
  DrawNodeLegend;
end;


procedure TMapForm1.LinkLegendBoxPaint(Sender: TObject);
//-----------------------------------
// OnPaint handler for LinkLegendBox.
// Redraws the Link Legend.
//-----------------------------------
begin
  if DraggedLink then exit;
  DrawLinkLegend;
end;


procedure TMapForm1.DrawNodeLegend;
//------------------------------------------
...
begin
  ...
  begin
    ...
    Umap.DrawLegend(NodeLegendBox.BoundsRect,
      ...
      MainForm.BrowserForm.NodeViewBox.Items[CurrentNodeVar],
      NodeLegend[CurrentNodeVar],
      MapNodeColor);
    NodeLegendPanel.Visible := True;
    {$IFDEF DARWIN} NodeLegendPanel.Repaint; {$ENDIF}
  end;
end;


procedure TMapForm1.DrawLinkLegend;
//------------------------------------------
...
begin
  ...
  begin
    ...
    Umap.DrawLegend(LinkLegendBox.BoundsRect,
      ...
      MainForm.BrowserForm.LinkViewBox.Items[CurrentLinkVar],
      LinkLegend[CurrentLinkVar],
      MapLinkColor);
    LinkLegendPanel.Visible := True;
    {$IFDEF DARWIN} LinkLegendPanel.Repaint; {$ENDIF}
  end;
end;
...
procedure TMapForm1.TimeLegendPanelDblClick(Sender: TObject);
//---------------------------------------------------
// OnDoubleClick event handler.
//---------------------------------------------------
begin
...
procedure TMapForm1.ToggleTimeLegend;
//---------------------------------------
...
procedure TMapForm1.CopyToBitmap(const Fname: String);
//-----------------------------------------------------------------
...
begin
  ...
  try
    ...
    aBitmap.Canvas.CopyRect(R, Canvas, R);
...
procedure TMapForm1.CopyToMetafile(const Fname: String);
//-------------------------------------------------------
...
begin
  ...
  MyDoc.Width := Map.Window.Pwidth;
  MyDoc.Height := Map.Window.Pheight;
  MyMetafile := MyDoc.AddPage(True);
...
{$IFDEF WINDOWS}
procedure TMapForm1.DrawToMetafile(aMetafile: TMetafile);
{$ELSE}
procedure TMapForm1.DrawToMetafile(aMetafile: TvVectorialPage);
{$ENDIF}
//------------------------------------------------------
    ...
    begin
      ...
      begin
        ...
        DrawTimeLegend(R, Canvas,
          MapBackColor[Options.ColorIndex],
                       {$IFDEF WINDOWS}
                       UTF8ToCP1252(TimeLegendPanel.Caption));
                       {$ELSE}
...
//Restore map's canvas & free the metafile canvas & clipping region
  ...
  {$ELSE}
  //aMetafile.Width := Map.Window.Pwidth;
  //aMetafile.Height := Map.Window.Pheight;
  //Add background to the file
  Background := TvRectangle.Create(aMetafile);
  Background.Brush.Color := TColorToFPColor(MapBackColor[Map.Options.ColorIndex]);
  Background.Brush.Style := bsSolid;
  Background.Pen.Color := TColorToFPColor(clBlack);
  if MapBackColor[Map.Options.ColorIndex] = clBlack then
     Background.Pen.Color := TColorToFPColor(clWhite);
  Background.X := 0;
  Background.Y := 0;
  Background.CX := Map.Window.Pwidth;
  Background.CY := Map.Window.Pheight;
  aMetafile.AddEntity(Background);

  try
  with Map do
  begin;
    //if MapBackdrop.Visible then DrawBackdropSVG(aMetafile);
    DrawNetworkSVG(aMetafile);
    SFx := Map.Window.Pwidth/ClientWidth - 1.0;
    SFy := Map.Window.Pheight/ClientHeight - 1.0;
    if NodeLegendPanel.Visible then
    begin
      R := NodeLegendPanel.BoundsRect;
      if OffsetRect(R, Round(R.Left*SFx), Round(R.Top*SFy)) then
      DrawLegendSVG(R, aMetafile, Canvas,
        MapBackColor[Options.ColorIndex],
        NodeLegendFrame.Framed,
        MainForm.BrowserForm.NodeViewBox.Items[CurrentNodeVar],
        NodeLegend[CurrentNodeVar],
    ...
    begin
      R := TimeLegendPanel.BoundsRect;
      if OffsetRect(R, Round(R.Left*SFx), Round(R.Top*SFy)) then
      DrawTimeLegendSVG(R, aMetafile, Canvas,
        MapBackColor[Options.ColorIndex],
                     {$IFDEF WINDOWS}
                     UTF8ToCP1252(TimeLegendPanel.Caption));
                     {$ELSE}
...
procedure TMapForm1.CopyToString(const Fname: String);
//-----------------------------------------------------
...
// Create a stringlist to hold value for each node
  ...
  // Add heading for each item
    with MainForm.BrowserForm.NodeViewBox do S := Items[ItemIndex];
    Slist.Add(TXT_MAP_NODE + #9 + 'X ' + #9 + 'Y ' + #9 + S);
...
procedure TMapForm1.Print(Destination: TDestination);
//----------------------------------------------------
...
// Begin job on thePrinter's destination (printer or print preview)
  ...
  // and draw it on the printed page
      ...
      MyMetafile.Free;
      {$ELSE}
      aPicture.Bitmap.Width := Map.Window.Pwidth + 2;
      aPicture.Bitmap.Height := Map.Window.Pheight + 2;
      MyMetafile.Render(aPicture.Bitmap.Canvas, 1, 1);
      StretchGraphic(FrameLeft, FrameTop, FrameLeft+MapWidth,
        FrameTop+MapHeight, aPicture);
    finally
      aPicture.Free;
